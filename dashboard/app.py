...
THEME_CSS="""[data-theme=light]{--b:#f3f6f4;--p:#ffffff;--l:#d7e1de;--t:#172522;--m:#60736f;--a:#087f68;--g:#b16b00}body[data-theme=light]{background:var(--b);color:var(--t)}body[data-theme=light] .side{background:#ffffff;border-color:var(--l)}body[data-theme=light] .side a{color:#38504b}body[data-theme=light] .side a:hover,body[data-theme=light] .side .on{background:#dff2ec!important;color:#123a31!important}body[data-theme=light] .head{border-color:var(--l)}body[data-theme=light] .head p,body[data-theme=light] .sub{color:#526762}body[data-theme=light] .filter,body[data-theme=light] .card{background:var(--p);border-color:var(--l)}body[data-theme=light] .filter select{background:#ffffff;border-color:#b7c8c3;color:var(--t)}body[data-theme=light] .tab th{background:#edf4f1;color:#405650}body[data-theme=light] .tab td{border-color:#e0e9e6}body[data-theme=light] .pill{background:#e5f4ef;color:#176452}.theme-toggle{position:fixed;right:22px;top:18px;z-index:10;border:1px solid var(--l);border-radius:999px;background:var(--p);color:var(--t);padding:8px 12px;font-weight:700;cursor:pointer;box-shadow:0 3px 12px #0002}.analysis-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;margin-top:18px}.analysis-grid .card{min-height:170px}.analysis-grid h3{margin:0 0 10px;font-size:15px}.analysis-grid p{margin:0;color:var(--m);line-height:1.65}.analysis-grid strong{color:var(--t)}@media(max-width:800px){.analysis-grid{grid-template-columns:1fr}.theme-toggle{right:12px;top:12px}}"""
THEME_SCRIPT="""<script>(function(){const key='togo-connect-theme';const body=document.body;const button=document.createElement('button');button.className='theme-toggle';button.type='button';button.setAttribute('aria-label','Changer de thème');function apply(theme){body.dataset.theme=theme;button.textContent=theme==='light'?'Mode sombre':'Mode clair';document.querySelectorAll('.js-plotly-plot').forEach(function(chart){if(window.Plotly){const light=theme==='light';Plotly.relayout(chart,{paper_bgcolor:light?'#ffffff':'#10181e',plot_bgcolor:light?'#ffffff':'#10181e','font.color':light?'#263b36':'#d6e1df','xaxis.gridcolor':light?'#dce8e4':'#304047','yaxis.gridcolor':light?'#dce8e4':'#304047'});}});localStorage.setItem(key,theme)}const saved=localStorage.getItem(key)||'dark';document.addEventListener('DOMContentLoaded',function(){document.body.appendChild(button);apply(saved);button.addEventListener('click',function(){apply(body.dataset.theme==='light'?'dark':'light')})})})()</script>"""
ANALYSIS_TEMPLATE="""<section class='analysis-grid'><article class='card'><h3>Ce que les chiffres racontent</h3>{%for title,text in findings%}<p><strong>{{title}}</strong> {{text}}</p>{%endfor%}</article><article class='card'><h3>Décisions à prendre</h3><p><strong>1. Cibler avant de déployer.</strong> Commencer par les communes à faible densité Mobile Money et forte population, puis vérifier la couverture réseau et la viabilité du site.</p><p><strong>2. Séparer les leviers.</strong> Un déficit d'agents n'appelle pas automatiquement un investissement dans une infrastructure télécom fixe.</p><p><strong>3. Compléter la mesure.</strong> Ajouter couverture 2G/3G/4G, qualité de service, fréquentation et distance au point le plus proche avant toute décision d'investissement.</p></article></section>"""
SIDEBAR_THEME_CSS=""".theme-toggle{position:static;display:flex;width:calc(100% - 32px);margin:18px 16px 0;justify-content:space-between;align-items:center;border:1px solid var(--l);border-radius:5px;background:transparent;color:var(--t);padding:11px 14px;font-weight:700;cursor:pointer}.theme-toggle:before{content:'◐';color:var(--a);font-size:17px}.theme-toggle:hover{background:#123e37;color:#fff}"""
SIDEBAR_THEME_SCRIPT="""<script>document.addEventListener('DOMContentLoaded',function(){var button=document.querySelector('.theme-toggle');var side=document.querySelector('.side');if(button&&side){side.appendChild(button)}})</script>"""
VISUAL_FIX_CSS="""body[data-theme=light] h1,body[data-theme=light] h2,body[data-theme=light] h3,body[data-theme=light] .value,body[data-theme=light] .item b,body[data-theme=light] .ins b{color:#172522}body[data-theme=light] .item,body[data-theme=light] .ins{color:#17352e}body[data-theme=light] .ins{background:#e5f4ef;border-left-color:#087f68}body[data-theme=light] .ins b{color:#123a31}body[data-theme=light] .eye,body[data-theme=light] .label,body[data-theme=light] .cap,body[data-theme=light] .foot{color:#40665c}body[data-theme=light] .filter button{color:#ffffff;background:#087f68}.map{min-height:620px;background:#e9efed;border-radius:4px;overflow:hidden}.map iframe{display:block;width:100%;height:620px;border:0}.tab{color:var(--t)}body[data-theme=light] .badge{color:#573b00;background:#ffe4a8}@media(max-width:800px){.map,.map iframe{min-height:480px;height:480px}}"""
...
def dash():
    ...
    coverage_fig=px.bar(r,x='region_nom_bdd',y=['mm','fixed'],barmode='group',labels={'region_nom_bdd':'Région','value':'Nombre de points','variable':'Type'},color_discrete_map={'mm':'#ee736a','fixed':'#599fff'});coverage_fig.update_layout(height=330)
    ...
    top=r.iloc[-1] if len(r) else None
    mobile_count=int((d.service=='Mobile money').sum())
    mobile_share=mobile_count/len(d)*100 if len(d) else 0
    findings=[]
    if low is not None and top is not None:
        gap=top.density-low.density
        findings.append(('Écart territorial.',f"{top.region_nom_bdd} atteint {top.density:.1f} points / 10 000 habitants, soit {gap:.1f} de plus que {low.region_nom_bdd}."))
    findings.append(('Nature de l’offre.',f"Le Mobile Money représente {mobile_share:.1f} % des points visibles dans le périmètre courant ({mobile_count:,} points).".replace(',', ' ')))
    findings.append(('Lecture prudente.',f"Les ratios reposent sur {len(c)} communes raccordées à la population RGPH-5; ils mesurent une présence recensée, pas la qualité ni la couverture du réseau."))
